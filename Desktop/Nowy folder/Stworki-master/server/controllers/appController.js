...
export async function getUserInfo(req, res){
    try {
        const userId = req.user.userId; // ID użytkownika z tokenu JWT
        const user = await UserModel.findById(userId).select('-password'); // Znajdź użytkownika i pomiń hasło
        if (!user) return res.status(404).send({ error: 'Użytkownik nie znaleziony' });
        res.status(200).send(user); // Zwróć dane użytkownika
